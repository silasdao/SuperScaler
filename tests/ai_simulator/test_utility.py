...
from superscaler.ai_simulator.simulator.utility import \
...
def test_transfer_rate_converter():
    ...
    for _ in range(10):
        ...
        rate_str = f'{str(rand_rate)}bps'
        assert rand_rate == transfer_rate_to_bps(rate_str)
    # Test kibps --> bps
    for _ in range(10):
        rand_rate = random.random() * 1000
        rate_str = f'{str(rand_rate)}Kibit/s'
...
def test_data_size_converter():
    ...
    for _ in range(10):
        ...
        size_str = f'{str(rand_size)}b'
        assert rand_size == data_size_to_bit(size_str)
    # Test kibit --> bit
    for _ in range(10):
        rand_size = random.random() * 1000
        size_str = f'{str(rand_size)}Kib'
